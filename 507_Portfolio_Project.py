...
## Steps below are in the code:
    
# 1) Produce a new file: totalfile.txt.
# 2) Numbers from each line of the two files.
# 3) Sum the lines and output to totalfile.txt.
# 4) Need to use concurrency. 
# 5) Record the amount of time it takes for each version of the program to complete the task.
# 6) Optimize the program by using threads to benefit from multiple cores in CPU.
# 7) Can I try using the GPU?
# 8) Now breakup both hugefiles into 10 files each
# 9) Run your process on all 10 sets in parallel

## Steps below are in the writing of the paper:
    
# 10) How do the run times compare to the original process?
# 11) Explain your methods and results in detail
# 12) What conclusions can you make about the different methods of optimizing large file processing?
# 13) How has the information that you learned helped you to accomplish this task?
